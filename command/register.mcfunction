...
execute as @e[type=item,scores={sxcscore=131586}] run tag @e[type=item,nbt=!{Item:{tag:{cmd:{cb:{type:[drop]}}}}},sort=nearest,limit=1] add i1
...
execute as @e[type=item,tag=i1] store result score @s t2 run data get entity @s Pos.[0] 256
execute as @e[type=item,tag=i1] store result score @s t3 run data get entity @s Pos.[0] 65536
...
execute as @e[type=item,scores={sxcscore=131586}] run scoreboard players reset @s sxcscore
